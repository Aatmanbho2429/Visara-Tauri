use crate::{
    ...
    core::sidecar,
    models::response::{ApiResponse, AuthUser, ResponseLogin, ResponsePeriodicRevalidate, ResponseValidateToken},
    services::license,
};
use keyring::Entry;
...
pub async fn login(email: &str, password: &str) -> ApiResponse<ResponseLogin> {
    ...
    if !data["success"].as_bool().unwrap_or(false) {
        return ApiResponse::err(401, data["message"].as_str().unwrap_or("Login failed"));
    }

    let token   = data["token"].as_str().unwrap_or_default();
    let user: AuthUser = match serde_json::from_value(data["user"].clone()) {
        Ok(u) => u,
        Err(_) => return server_error(),
    };

    save_token(token);
    set_session(&user.id, &data["user"]);
    sidecar::maybe_notify_ready(); // sidecar may already be healthy and waiting on this login

    ApiResponse::ok_with_message(
        data["message"].as_str().unwrap_or("Login successful"),
        ResponseLogin { token: token.to_string(), user },
    )
}

// Called on app startup: validates the saved token, confirms the
// subscription is current, and stores user info in memory.
pub async fn validate_saved_token() -> ApiResponse<ResponseValidateToken> {
    ...
    if !data["valid"].as_bool().unwrap_or(false) {
        ...
        return ApiResponse::err(
            401,
            data["message"].as_str().unwrap_or("Session expired. Please login again."),
        );
    }
    ...
    let status = user.subscription_status.as_str();
    ...
    ApiResponse::ok_with_message("Session valid", ResponseValidateToken { user })
}

pub fn logout() -> ApiResponse<()> {
    delete_token();
    clear_in_memory_session();
    ...
    std::thread::spawn(sidecar::clear_model_key);
    sidecar::reset_notified();
    ApiResponse::ok_empty("Logged out successfully")
}
...
// Background re-validation tick. `action` is one of:
...
//                     the user out.
pub async fn periodic_revalidate() -> ApiResponse<ResponsePeriodicRevalidate> {
    if saved_token().is_none() {
        return ApiResponse::ok_with_message(
            "No session",
            ResponsePeriodicRevalidate { action: "none".to_string(), user: None },
        );
    }
    ...
    if result.status_code == 200 {
        let user = result.data.map(|d| d.user);
        return ApiResponse::ok_with_message(
            "Session re-validated",
            ResponsePeriodicRevalidate { action: "ok".to_string(), user },
        );
    }
    ...
    if saved_token().is_none() {
        return ApiResponse::ok_with_message(
            "Session expired",
            ResponsePeriodicRevalidate { action: "logout".to_string(), user: None },
        );
    }
    ...
    if now_secs() - last_valid < OFFLINE_GRACE_SECS {
        return ApiResponse::ok_with_message(
            ...
            ResponsePeriodicRevalidate { action: "ok-offline".to_string(), user: None },
        );
    }
...
pub async fn send_otp(email: &str) -> ApiResponse<()> {
    let result = reqwest::Client::new()
...
pub async fn forgot_password_send_otp(email: &str) -> ApiResponse<()> {
    let result = reqwest::Client::new()
...
pub async fn forgot_password_verify_otp(email: &str, otp_code: &str) -> ApiResponse<()> {
    let result = reqwest::Client::new()
...
pub async fn change_password(old_password: &str, new_password: &str) -> ApiResponse<()> {
    let token = match saved_token() {
        Some(t) => t,
...
// Instant check — no network call. `Ok` (200) if a token file/keychain entry
// exists on disk, 401 if the user has never logged in or has logged out.
// Used by route guards to avoid a Supabase round-trip on every navigation.
pub fn check_session() -> ApiResponse<()> {
    if saved_token().is_some() {
        ApiResponse::ok_empty("Session exists")
    } else {
        ApiResponse::err(401, "No session")
    }
}

// ── Error / passthrough helpers ──────────────────────────────────────────

// Several Supabase edge functions (OTP, password reset/change) are simple
// enough that we just relay their own `{success, message}` shape into our
// envelope rather than typing a bespoke response for each — there is no
// payload beyond the message on any of these.
async fn passthrough(result: Result<reqwest::Response, reqwest::Error>) -> ApiResponse<()> {
    let resp = match result {
        Err(e) => return network_error(e),
        Ok(r) => r,
    };
    ...
    if data["success"].as_bool().unwrap_or(false) {
        ApiResponse::ok_empty(if message.is_empty() { "OK".to_string() } else { message })
    } else {
        ApiResponse::err(400, if message.is_empty() { "Request failed".to_string() } else { message })
    }
}

fn server_error<T>() -> ApiResponse<T> {
    ApiResponse::err(500, "Invalid response from server")
}
