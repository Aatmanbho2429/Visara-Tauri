...
// tag-based querying.  Each public fn returns an `ApiResponse<T>`, matching
// the other services.

use crate::{
    core::{database, search_gate, sidecar},
    models::response::{ApiResponse, ResponseTagFacets, ResponseTagQuery, ResponseTagSuggest, ResponseTagsGet, TagSuggestionDto},
};
...
pub fn set_tags(paths: Vec<String>, category: String, value: String) -> ApiResponse<()> {
    let value = value.trim().to_string();
    if value.is_empty() {
        return ApiResponse::err(422, "Tag value cannot be empty.");
    }
    ...
    ApiResponse::ok_empty(format!("Tagged {ok} image{}", if ok == 1 { "" } else { "s" }))
}

// Remove a specific tag from every selected image.
pub fn remove_tag(paths: Vec<String>, category: String, value: String) -> ApiResponse<()> {
    let con = match database::open() {
        Ok(c) => c,
        Err(e) => return ApiResponse::err(500, format!("Database error: {e}")),
    };
    ...
    ApiResponse::ok_empty("Tag removed.")
}

// ── Reads ────────────────────────────────────────────────────────────────────

// All tags for a set of paths (used by the Categorize panel to show current tags).
pub fn get_tags(paths: Vec<String>) -> ApiResponse<ResponseTagsGet> {
    match database::open().and_then(|c| database::tags_for_paths(&c, &paths)) {
        Ok(tags) => ApiResponse::ok(ResponseTagsGet { tags }),
        Err(e) => ApiResponse::err(500, format!("Could not read tags: {e}")),
    }
}

// Distinct (category, value) tags with counts — drives the filter chips.
pub fn facets() -> ApiResponse<ResponseTagFacets> {
    match database::open().and_then(|c| database::tag_facets(&c)) {
        Ok(facets) => ApiResponse::ok(ResponseTagFacets { facets }),
        Err(e) => ApiResponse::err(500, format!("Could not read tags: {e}")),
    }
}

// Paths matching ALL of the given (category, value) filters.
pub fn query(filters: Vec<(String, String)>) -> ApiResponse<ResponseTagQuery> {
    match database::open().and_then(|c| database::query_paths_by_tags(&c, &filters)) {
        Ok(paths) => ApiResponse::ok(ResponseTagQuery { paths }),
        Err(e) => ApiResponse::err(500, format!("Could not query tags: {e}")),
    }
}

// Suggested tags for a selection, derived from file/folder names (not pixels).
pub fn suggest(paths: Vec<String>) -> ApiResponse<ResponseTagSuggest> {
    ...
    let suggestions: Vec<TagSuggestionDto> = set
        .into_iter()
        .map(|(category, value)| TagSuggestionDto { category, value })
        .collect();
    ApiResponse::ok(ResponseTagSuggest { suggestions })
}
