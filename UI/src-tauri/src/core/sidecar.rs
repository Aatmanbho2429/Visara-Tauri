...
use crate::{
    ...
    models::response::{ApiResponse, ResponseSidecarCrashed},
    services::auth,
};
...
fn watchdog(app: AppHandle) {
    ...
    loop {
        ...
        if consecutive_crashes > MAX_CONSECUTIVE_CRASHES {
            ...
            let payload = ApiResponse::ok(ResponseSidecarCrashed {
                recovering: false,
                message: "The AI engine keeps failing to start. Try restarting Pictoria.".to_string(),
            });
            let _ = app.emit("sidecar_crashed", payload);
            return;
        }
        ...
        let payload = ApiResponse::ok(ResponseSidecarCrashed {
            recovering: true,
            message: "The AI engine stopped unexpectedly — restarting it now…".to_string(),
        });
        let _ = app.emit("sidecar_crashed", payload);
...
pub fn push_model_key() {
    ...
    // Read status + raw body *before* attempting to parse it as JSON. A plain
    // `r.json()` consumes the response and, on failure, only reports serde's
    // generic decode error — not what actually came back. That's not enough
    // to tell "Flask returned an HTML error page", "something on the loopback
    // truncated/corrupted the body" (a real, previously-seen failure mode on
    // this port — see `/verify`'s history), and "the JSON parsed but didn't
    // match `ModelResp`" apart from each other, and this call is exactly the
    // one worth telling apart: it's the slowest request in the app (up to
    // `MODEL_LOAD_TIMEOUT`), the one most exposed to that kind of interference,
    // and a wrong verdict here silently blocks every search until the next
    // token-validate retries it.
    let parsed: ModelResp = match resp {
        Ok(r) => {
            let status = r.status();
            let body = r.text().unwrap_or_else(|e| format!("<could not read body: {e}>"));
            match serde_json::from_str::<ModelResp>(&body) {
                Ok(v) => v,
                Err(e) => {
                    let snippet: String = body.chars().take(500).collect();
                    log::error!(
                        "[sidecar] /model bad response: status={status} parse_error={e} \
                         body_len={} body_snippet={snippet:?}",
                        body.len(),
                    );
                    return;
                }
