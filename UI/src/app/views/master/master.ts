...
export class Master implements OnInit, OnDestroy {
  ...
  private revalidateSession(): void {
    this.auth.periodicRevalidate().subscribe({
      next: data => {
      ...
      // A network-level failure surfaces as an ApiError too (see AuthService);
      // there's nothing actionable here — the next timer tick tries again.
      error: () => {},
    });
  }
  ...
  private maybeShowMigrationTip(): void {
    ...
    this.libSvc.stats().subscribe({
      next: data => {
        const { watchedFolderCount, totalIndexedFiles } = data;
        if (watchedFolderCount === 0 && totalIndexedFiles > 0) {
          this.messages.add({
            key: 'app',
            severity: 'info',
      ...
      error: () => { localStorage.setItem(Master.LIBRARY_MIGRATE_KEY, '1'); },
    });
  }
