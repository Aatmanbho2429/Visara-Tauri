...
class Embedder:
    """Singleton ONNX CLIP inference session. Decrypted in-memory after login."""
    _instance  = None
    ...
    def set_key(self, key: str):
        """Called after successful login/token-validate with decryption key from Supabase."""
    ...
    def _setup(self, key: str):
        ...
        model_bytes = Fernet(key.encode()).decrypt(encrypted_bytes)
    ...
    def embed_batch(self, batch: np.ndarray) -> np.ndarray:
        ...
        if raw.shape[0] == 1 and batch.shape[0] > 1:
            ...
            for i in range(batch.shape[0]):
                with self._lock:
                    out = self._session.run([self._out_name], {self._in_name: batch[i:i+1]})[0]
    ...
    @property
    def std(self) -> np.ndarray:  return self._std
